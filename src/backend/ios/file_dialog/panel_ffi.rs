...
impl Panel {
    ...
    pub fn open_panel(content_types: Id<NSArray<NSString>>) -> Self {
        //let documentPicker = UIDocumentPickerViewController(forOpeningContentTypes: [.heic, .heif, .livePhoto, .image])
        unsafe {
            println!("Creating UIDocumentPickerViewController");
        let tmp: *mut Object = msg_send![class!(UIDocumentPickerViewController), alloc];
        //println!("{}", tmp);
        println!("UIDocumentPickerViewController::initForOpeningContentTypes");
        let tmp2: *mut Object = msg_send![tmp, initForOpeningContentTypes: content_types];
        return Self::new(tmp2);
        }
    ...
    pub fn filters_array(params: &FileDialog) -> Id<NSArray<NSString>> {
        if params.filters.is_empty() {
            println!("Empty filters...");
    ...
    pub fn set_parent(&self, parent: &RawWindowHandle) {
        let id = UIWindow::from_raw_window_handle(parent);
        unsafe {
...
impl Panel {
    pub fn build_pick_file(opt: &FileDialog) -> Self {
        let panel = Panel::open_panel(Panel::filters_array(&opt));
    ...
    pub fn build_pick_files(opt: &FileDialog) -> Self {
        let panel = Panel::open_panel(Panel::filters_array(&opt));
        println!("UIDocumentPickerViewController created!!");

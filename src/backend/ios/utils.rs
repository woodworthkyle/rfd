...
pub use application::{IUIApplication, UIApplication};
pub use focus_manager::FocusManager;
pub use policy_manager::PolicyManager;
pub use url::{INSURL, NSURL};
pub use window::{IUIWindow, UIWindow};
...
pub fn run_on_main<R: Send, F: FnOnce() -> R + Send>(run: F) -> R {
    if is_main_thread() {
        ...
        println!("We are not on the main thread! Trying to get there via UIApplication::sharedApplication()");
